...
from WebApp.auth.forms import AccountUpdateForm, LoginForm, SignupForm
...
def signup():
    ...
    if request.method == "POST" and form.validate_on_submit():
        ...
        flash(
            f"Ο λογαριασμός για τον χρήστη <b>{username}</b> δημιουργήθηκε με επιτυχία",
            "success",
        )
...
def login():
    ...
    if request.method == "POST" and form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, password):
            flash(
                f"Η είσοδος του χρήστη με email: {email} στη σελίδα μας έγινε με επιτυχία.",
                "success",
            )
            ...
            flash(
                "Η είσοδος του χρήστη ήταν ανεπιτυχής, παρακαλούμε δοκιμάστε ξανά με τα σωστά email/password.",
                "warning",
            )
...
def account():
    ...
    if request.method == "POST" and form.validate_on_submit():
        ...
        if form.profile_image.data:
            try:
                image_file = image_save(
                    form.profile_image.data, "profiles_images", (150, 150)
                )
            except:
                abort(415)
        ...
        flash(
            f"Ο λογαριασμός του χρήστη <b>{current_user.username}</b> ενημερώθηκε με επιτυχία",
            "success",
        )
